<?php 
	...
	$app->any('/','getRandomPractices');
	$app->run();

	function getRandomPractices() {
		global $app, $crypt;
        $request = $app->request();
        if ( !$request->isPost() ) {
        ...
        $rqid = $request->params('rqid');
        $validate_result = validate(array('license'=>'NOT_NULL', 'subject'=>'INT', 'rqid' => 'INT'), $request->params());
        ...
        try{
        	$db = getConnection();
        	$sql = "SELECT * FROM `cs_exams` WHERE `ctype` = '{$license}' AND `stype` = '{$subject}' ORDER BY {$rqid}*rand()";
        	$stmt = $db->query($sql);
