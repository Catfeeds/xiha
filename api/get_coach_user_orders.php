	...
	function getOrders($id, $status_id) {
		...
		try {
			...
			foreach ($order_list as $key => $value) {
				...
				// 获取学员头像
				$sql = "SELECT i.`user_photo`, i.`identity_id`, u.`s_real_name` FROM `cs_user` as u";
			...
			// 	}
			// 	$order_list[$key]['time_config_arr'] = $_time;
			// 	$order_list[$key]['s_student_imgurl'] = $imgurl['user_photo'] == null ? '' : $imgurl['user_photo'];
			...
			// 	if($row) {
			// 		$order_list[$key]['is_comment'] = 1;
			// 	} else {
			// 		$order_list[$key]['is_comment'] = 2;
			// 	}
