<?php  
	 ...
	 * @param $date 预约的时间 
	 * @param $money 总价钱 
	 * @param $type 交易类型 1:支付宝 2：线下支付 3：微信 4：银联
	...
	function takeOrders() {
		...
		$month = $date[0];
		$day = $date[1];
		$allowed_pay_type = array(1, 3, 4);  // 1 支付宝 2 线下 3 微信 4 银联

		$db = getConnection();
		// 获取驾校ID
		$sql = "SELECT `s_school_name_id` FROM `cs_coach` WHERE `l_coach_id` = '{$coach_id}'";
		$stmt = $db->query($sql);
		$_school_info = $stmt->fetch(PDO::FETCH_ASSOC);
		$school_id = 0;
		if($_school_info) {
			$school_id = $_school_info['s_school_name_id'];
		}

		// 单独给万达驾校开通线下支付
		if($school_id != 230) {

			if (!in_array($request->params('type'), $allowed_pay_type) && $dc_money != 0) {
	            //线下
	            echo json_encode( array('code'=>109, 'data'=>'线下支付已关闭')  );
	            return;
	        }
		}
		...
		try {
			...
			// $order_sn = $year_code[intval(date('Y'))-2, 'data'=>''0;10].strtoupper(dechex(date('m'))).date('d').substr(time(),-5).substr(microtime(),2,5).sprintf('d',rand(0,99));
			...
			$params_coach = array(
				...
				'title'=>'预约学车订单',
				'content'=>$coachinfo['s_coach_name'].'教练您好，'.$message_username.'学员(手机：'.$userinfo['s_phone'].')预约您'.$month.'月'.$day.'日'.implode(',', $time_config_arr).'点的学车服务(预约订单号为：'.$s_order_no.')，请提前做好安排!',
				'type'=>2 //(1:学员端 2：教练端)
			);
			...
			$params_student = array(
				...
				'i_yw_type'=>2, // 1:通知 2：正常订单消息
				'title'=>'预约学车订单',
				'content'=>$userinfo['s_real_name'].'学员您好，您已成功预约'.$s_school_name.$coachinfo['s_coach_name'].'教练(手机：'.$coachinfo['s_coach_phone'].')'.$month.'月'.$day.'日'.implode(',', $time_config_arr).'时的学车服务(预约订单号为：'.$s_order_no.')，请准时到达学车地点，提前和教练电话沟通。',
				'type'=>2 //(1:学员端 2：教练端)
			);
