 ...
 * @param string $lesson_type 科目一/科目四
 * @param string $licence_type 牌照:C1 
 * @param string $coach_id 教练ID
...
function getCoachDetail() {
    ...
    if (!$request->isPost()) {
        ...
        slimLog($request, $response, null, '未使用POST发送请求');
        ajaxReturn($data);
    }
    ...
    try {
        ...
        if($coach_info) {
            ...
            // 获取所带学员总数
            $sql = "SELECT count(`l_study_order_id`) as num FROM `cs_study_orders` WHERE `l_coach_id` = $id AND `i_status` = 2"; // i_status=2-已完成订单
            $stmt = $db->query($sql);
            ...
            if($coach_info->s_coach_car_id) {
                ...
                if (trim($coach_info->s_coach_imgurl) != '') {
            ...
            if($_coach_info) {
                ...
                $s_am_time_list = isset($_coach_info['s_am_time_list']) ? array_filter(explode(',', $_coach_info['s_am_time_list'])) : array(); 
                $s_pm_time_list = isset($_coach_info['s_pm_time_list']) ? array_filter(explode(',', $_coach_info['s_pm_time_list'])) : array();
                $s_coach_lisence_id_list = isset($_coach_info['s_coach_lisence_id']) ? array_filter(explode(',', $_coach_info['s_coach_lisence_id'])) : array();
                $s_coach_lesson_id_list = isset($_coach_info['s_coach_lesson_id']) ? array_filter(explode(',', $_coach_info['s_coach_lesson_id'])) : array();
            }
            ...
            if($current_coach_time_config_arr) {
                ...
                foreach ($_time_config_list as $key => $value) {
                    ...
                    // 牌照按照基础信息设置
                    $s_coach_lisence_id_list = array_filter($s_coach_lisence_id_list);
                    $s_coach_lesson_id_list = array_filter($s_coach_lesson_id_list);
                    if(count($s_coach_lisence_id_list) == 1) {
                ...
                $db = null;
                // 对价格进行优化处理，大于1元的取整
                if (is_array($coach_time_list) && count($coach_time_list) > 0) {
                    foreach ($coach_time_list as $coach_time_index => $coach_time) {
                        if (isset($coach_time['time_list']) && count($coach_time['time_list']) > 0) {
                            foreach ($coach_time['time_list'] as $time_index => $time) {
                                // price
                                if ($time['price'] >= 1) {
                                    $coach_time_list[$coach_time_index]['time_list'][$time_index]['price'] = round($time['price']);
                                }

                                // final_price
                                if ($time['final_price'] >= 1) {
                                    $coach_time_list[$coach_time_index]['time_list'][$time_index]['final_price'] = round($time['final_price']);
                                }
                            } // 一天结束
                        }
                    } // 多天结束
        ...
        $db = null;
        slimLog($app->request(), $app->response(), $e, 'pdo数据库异常');
        $data = array('code'=>1, 'data'=>'网络错误');
        ajaxReturn($data);
    } catch(ErrorException $e) {
        $db = null;
        slimLog($app->request(), $app->response(), $e, 'slim应用解析异常');
