...
function getCoachlist() {
    ...
    try {
        ...
        // 获取当前教练列表经纬度
        $sql = "SELECT `dc_coach_distance_x`, `dc_coach_distance_y`, `l_coach_id` FROM `cs_coach` AS c LEFT JOIN `cs_user` AS u ON c.user_id = u.l_user_id WHERE u.i_status = 0 AND u.i_user_type = 1 AND `s_coach_lesson_id` LIKE '%".$lesson_id."%' AND `s_coach_lisence_id` LIKE '%".$lisence_id."%' AND `city_id` = ".$city_id;
        // 循环保存数据
        $stmt = $db->query($sql);
    ...
    // 获取当前列表根据综合排序
    $sql = "SELECT s.`name` as `s_coach_car_name`, s.`car_type`, h.* FROM `cs_coach` as h LEFT JOIN `cs_user` AS u ON h.user_id = u.l_user_id LEFT JOIN `cs_cars` as s ON s.`id` = h.`s_coach_car_id` WHERE u.i_user_type = 1 AND u.i_status = 0 AND h.`s_coach_lesson_id` LIKE '%".$lesson_id."%' AND h.`s_coach_car_id` != '' AND h.`s_coach_lisence_id` LIKE '%".$lisence_id."%' AND h.`city_id` = '".$city_id ."' AND h.`order_receive_status` = 1";
    ...
    if($type == 'default') {
        ...
        try {
            ...
            foreach ($coach as $key => $value) {
                ...
                $coach[$key]['hot_params'] = getHotParams($value['i_type']);
        ...
        try {
            ...
            if($coach) {
                ...
                $coach = array_slice($coach, $start, $limit);
            } else {
                $coach = array();
            }
            $db = null;
        ...
        $sql .= " WHERE h.`s_coach_lesson_id` LIKE '%".$lesson_id."%' AND h.`s_coach_lisence_id` LIKE '%".$lisence_id."%' AND h.`city_id` = ".$city_id ." AND h.`order_receive_status` = 1 AND h.`l_coach_id` IN (".implode(',', $coach_ids).")";
...
function saveDistance($distance, $coach_id, $school_id) {
    try {
        ...
        $sql = "SELECT `time_config_money_id` FROM `cs_current_coach_time_configuration` WHERE `year` = '{$year}' AND `month` = '{$month}' AND `day` = '{$day}'";
        $stmt = $db->query($sql);
        $time_config_money_id_info = $stmt->fetch(PDO::FETCH_ASSOC);
        if($time_config_money_id_info) {
            $time_config_money_id_arr = json_decode($time_config_money_id_info['time_config_money_id'], true);
            // print_r($time_config_money_id_arr);
            $price_arr_count = count($time_config_money_id_arr);
            if($time_config_money_id_arr) {
                foreach ($time_config_money_id_arr as $key => $value) {
                    $price_arr_total_num += $value;
                }
            }
            //$price_arr_average = round($price_arr_total_num / $price_arr_count);
            if (intval($price_arr_count) != 0) {
                $price_arr_average = ($price_arr_total_num / $price_arr_count);
            }
        } else {
            $sql = "SELECT `price` FROM `cs_coach_time_config` WHERE `school_id` = ".$school_id;
            $stmt = $db->query($sql);
            $price_arr = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $price_arr_total_num = 0;
            if($price_arr) {
