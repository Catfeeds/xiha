<?php 
	...
	$app->any('/','getEmulationPractices');
	$app->run();

	function getEmulationPractices() {
		global $app, $crypt;
        $request = $app->request();

        //configure emulation questions total number
        $limit = 100;
        ...
        try{
        	$db = getConnection();
        	$sql = "SELECT * FROM `cs_exams` WHERE `ctype` = '{$license}' AND `stype` = '{$subject}' ORDER BY rand() limit {$limit}";
        	$stmt = $db->query($sql);
