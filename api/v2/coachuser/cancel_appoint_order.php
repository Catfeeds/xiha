<?php 
     ...
     * @param $cancel_type 1：学员取消 2：教练取消
        ...
        try {
            ...
            if($study_order_info) {
                ...
                if($appoint_time > $time) {
                    ...
                    if($study_order_info['i_status'] == '1003') {
                ...
                if($res) {
                    ...
                    if($type == 1) {
                        ...
                        $params_coach = array(
                            ...
                            'title'=>'预约学车订单',
                            'content'=>'教练您好，您的'.$_appoint_time_m.'月'.$_appoint_time_d.'日'.$_appoint_time_h.':00时开始的订单'.$order_no.'（订单号）已被学员取消，原因是: '.$push_info['cancel_reason'].'，请您及时调整您的教学计划。该学员的姓名为 ' . $push_info['student_name'] . ',手机号为 ' . $push_info['student_phone'] . '。',
                        ...
                        $params_student = array(
                            ...
                            'i_yw_type'=>2, // 1:通知 2：正常订单消息
                            'title'=>'预约学车订单取消',
                            'content'=>'学员您好！您的'.$_appoint_time_m.'月'.$_appoint_time_d.'日'.$_appoint_time_h.':00时学车业务已取消成功，请和教练电话确认。该教练姓名为 ' . $push_info['coach_name'] . ', 手机号为 ' . $push_info['coach_phone'] . '。',
                    ...
                    // 教练端取消
                        ...
                        $params_coach = array(
                            ...
                            'i_yw_type'=>2, // 1:通知 2：正常订单消息
                            'title'=>'预约学车订单',
                            'content'=>'教练您好，您的'.$_appoint_time_m.'月'.$_appoint_time_d.'日'.$_appoint_time_h.':00时开始的订单已取消成功，请及时电话告知学员。该学员姓名为 '.$push_info['student_name'] . ', 手机号为 '.$push_info['student_phone'].'。',
                            'type'=>2 // 2:教练端推送 1：学员端推送
                        );
                        $res = request_post(SHOST.'api/message_push.php', $params_coach);
                        ...
                        $params_student = array(
                            ...
                            'i_yw_type'=>2, // 1:通知 2：正常订单消息
                            'title'=>'预约学车订单取消',
                            'content'=>'学员您好！您的'.$_appoint_time_m.'月'.$_appoint_time_d.'日'.$_appoint_time_h.':00时开始的订单'.$order_no.'（订单号）已被教练取消，原因是: '.$push_info['cancel_reason'].'，请您及时调整您的出行计划。该教练姓名为 ' .$push_info['coach_name'].', 手机号为 '.$push_info['coach_phone'].'。',
                            'type'=>1 // 2:教练端推送 1：学员端推送
                        );
