    ...
    function random() {
        ...
        try {
            ...
            if ($redis) {
                ...
                if ($redis->exists("exam:random_id:{$ctype}:{$stype}")) {
                    $data['code'] = 200;
                    $data['data'] = unserialize($redis->get("exam:random_id:{$ctype}:{$stype}"));
                    ajaxReturn($data);
                }
            }
            // redis
            ...
            $db = getConnection();
            $tbl = DBPREFIX . 'exams';
            $sql = " SELECT `id` FROM `{$tbl}` WHERE `ctype` = :c AND `stype` = :s ORDER BY RAND() * (SELECT MAX(id) FROM `{$tbl}`) ";
            $stmt = $db->prepare($sql);
            ...
            // if Redis is up
            if ($redis_already_up) {
                $redis->set("exam:random_id:{$ctype}:{$stype}", serialize($data['data']), $redis_conf['TTL_DAY']);
            }
