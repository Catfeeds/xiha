        ...
        if (!empty($user_info)) {
            ...
            if ($phone_user) {
                ...
                if ($user_type == 0) {
                    ...
                    if (empty($usersinfo)) {
                        $sql = "INSERT INTO `{$users_info_tbl}` (`user_id`, `identity_id`, `address`, `school_id`, `sex`, `age`) VALUES ('{$l_user_id}', '未填写身份证', '未填写地址', 0, 1, 0)";
                        $user = $db->query($sql);
                        if ($user) {
                    ...
                    if (empty($coach_message)) {
                        $sql = "INSERT INTO {$coach_tbl} (`s_coach_phone`, `user_id`, `s_coach_name`, `s_teach_age`, `s_coach_sex`, `s_school_name_id`, `s_coach_address`, `order_receive_status`, `addtime`) VALUES ('{$user_phone}', '{$l_user_id}', '{$user_name}', 0, 1, 0, '未填写地址', 0, '{$addtime}') ";
                        $user = $db->query($sql);
                        if ($user) {
                ...
                if ($user_type == 0) {
                    $sql = "INSERT INTO `{$users_info_tbl}` (`user_id`, `identity_id`, `address`, `school_id`, `sex`, `age`) VALUES ('{$user_id}', '未填写身份证', '未填写地址', 0, 1, 22)";
                    $user = $db->query($sql);
                } else if ($user_type == 1) {
                    $sql = "INSERT INTO {$coach_tbl} (`s_coach_phone`, `user_id`, `s_coach_name`, `s_teach_age`, `s_coach_sex`, `s_school_name_id`, `s_coach_address`, `order_receive_status`, `addtime`) VALUES ('{$user_phone}', '{$user_id}', '{$user_name}', 0, 1, 0, '未填写地址', 0, '{$addtime}') ";
                    $user = $db->query($sql);
                    if ($user) {
                        $coach_id = $db->lastInsertId();
                    }
