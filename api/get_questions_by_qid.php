<?php 
	...
	function getAllPracticesInTurn() {
        ...
        //	获取请求参数并判断合法性
        $qid_list = $request->params('qid_list');//第几条
        $validate_result = validate(array('qid_list'=>'STRING'), $request->params());
        ...
        try{
        	$db = getConnection();
            $collection = array_filter(explode(',', $qid_list));
            if(empty($collection)) {
                $data = array('code'=>104, 'data'=>'暂无错题');
                echo json_encode($data);
                exit();
            }
            $sql = "SELECT * FROM `cs_exams` WHERE `id` IN (".implode(',', $collection).") ORDER BY `id` DESC "; 
        	$stmt = $db->query($sql);
            ...
            setapilog('get_all_practices: params[qid_list:'.$qid_list.'],error:' . $e->getMessage());
