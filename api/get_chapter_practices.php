<?php 
     ...
     * @param integer $cid 章节id 1/2/3/4 
	...
	$app->any('/','getChapterPractices');
	$app->run();

	function getChapterPractices() {
		global $app, $crypt;
        $request = $app->request();
        if ( !$request->isPost() ) {
            $data = array('code' => 106, 'data' => '请求错误');
            setapilog('[get_chapter_practices] [:error] [client ' . $request->getIp() . '] [Method % ' . $request->getMethod() . '] [106 错误的请求方式]');
        ...
        $chapterid = $request->params('cid');//章节id 1/2/3/4
        $number = $request->params('number');//第几条
        $validate_result = validate(array('license'=>'NOT_NULL', 'subject'=>'INT', 'cid'=>'INT'), $request->params());
        ...
        try{
            ...
            if ($count) {
                $_limit = $c - $number;
            	$sql = "SELECT * FROM `cs_exams` WHERE `ctype` = '{$license}' AND `stype` = '{$subject}' AND `chapterid` = '{$chapterid}' ORDER BY `id` DESC ";
            ...
            setapilog('get_chapter_practices: params[license: ' . $license . ', subject: '.$subject.', cid: '.$cid.'],error:' . $e->getMessage());
