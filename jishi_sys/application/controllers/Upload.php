...
class Upload extends CI_Controller {
    ...
    public function handle()
    {
        ...
        if ( ! $this->upload->do_upload('file'))
        {
            ...
            $this->output->set_content_type('application/json')
                         ->set_output(json_encode(['code' => 400, 'msg' => $error['error'], 'data' => new \stdClass]))
                         ->_display();
            exit();
        }
        else
        {
            ...
            $this->output->set_content_type('application/json')
                         ->set_output(json_encode(['code' => 200, 'msg' => 'upload ok', 'data' => ['file_id' => $file_id, 'file_url' => base_url('upload/'.$_relative_path)]]))
                         ->_display();
            exit();
        }
    }
