...
class Securityguard extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('msecurityguard');
    }

    // 首页列表
    public function index()
    {
        $data['breadcrumb'] = ['首页', '驾培机构', '安全员'];
        $data['title'] = $this->title;
        $data['theme'] = $this->theme;
        $data['js_list'] = ['securityguard/index'];
        $data['list'] = $this->msecurityguard->list(1, 10);

        $this->load->view('templates/head', $data);
        $this->load->view('templates/top', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('securityguard/index', $data);
    ...
    public function add()
    {
        log_message('info', '访问securityguard/add');
        ...
        if ($this->input->method(TRUE) === 'POST')
        {
            if (empty($_POST))
            {
                ...
                $fields = $this->msecurityguard->getFields();
            ...
            {
                log_message('info', '访问securityguard/add 通过ajax');
                ...
                else
                {
                    ...
                    $result = $this->msecurityguard->add($_POST);
                    if ($result)
                    {
            ...
            {
                log_message('info', '访问securityguard/add 通过cli');
                // 未实现
            }
            else
            {
                log_message('info', '访问securityguard/add 通过form表单');
            }
        }
        else
        {
            log_message('info', '访问securityguard/add 渲染表单');

            $data['title'] = $this->title;
            $data['theme'] = $this->theme;
            $data['js_list'] = ['securityguard/add'];

            $this->load->view('templates/head', $data);
            $this->load->view('templates/top', $data);
            // $this->load->view('templates/menu', $data);
            $this->load->view('securityguard/add', $data);
            $this->load->view('templates/foot', $data);
        }
    }
    ...
    public function edit() {
        log_message('info', '访问securityguard/edit');
        ...
        if ($this->input->method(TRUE) === 'POST')
        {
            ...
            $this->form_validation->set_rules('secuid', '', 'required', ['required' => '安全员id未获取到']);
            $this->form_validation->set_rules('name', '', 'required', ['required' => '姓名未填写']);
            $this->form_validation->set_rules('sex', '', 'required', ['required' => '性别未选择']);
            $this->form_validation->set_rules('idcard', '', 'required|exact_length[18]', ['required' => '身份证号未填写', 'exact_length' => '身份证必须是18位']);
            $this->form_validation->set_rules('mobile', '', 'required|exact_length[11]', ['required' => '手机联系方式未填写', 'exact_length' => '手机号必须是11位']);
            ...
            if ($this->input->is_ajax_request())
            {
                log_message('info', '访问securityguard/edit 通过ajax');
                if ($this->form_validation->run() === FALSE)
                {
                    $errors = $this->form_validation->error_array();
                    if (isset($errors[array_keys($errors)[0]]))
                ...
                else
                {
                    ...
                    $id = (int) $this->input->post($this->msecurityguard->getPrimaryKey(), TRUE);
                    if ($id <= 0)
                    {
                    ...
                    else
                    {
                        ...
                        $result = $this->msecurityguard->update($_POST, $id);
                        if ($result)
                        {
            ...
            elseif ($this->input->is_cli_request())
            {
                log_message('info', '访问securityguard/edit 通过cli');
                // 未实现
            }
            else
            {
                log_message('info', '访问securityguard/edit 通过form表单');
            }
        }
        else
        {
            log_message('info', '访问securityguard/edit 渲染表单');

            $data['title'] = $this->title;
            $data['theme'] = $this->theme;
            $data['js_list'] = ['securityguard/edit'];

            $this->load->view('templates/head', $data);
            $this->load->view('templates/top', $data);
            // $this->load->view('templates/menu', $data);
            $this->load->view('securityguard/edit', $data);
            $this->load->view('templates/foot', $data);
        }
    ...
    {
        log_message('info', '访问 securityguard/list');
        ...
        if ($this->input->method(TRUE) === 'GET')
        {
            ...
            $list = $this->msecurityguard->list($limit, $offset);
            $total = $this->msecurityguard->total();
            ...
            {
                log_message('error', '不允许的请求 securityguard/list 非ajax');
            }
        }
        else
        {
            log_message('error', '不允许的请求 securityguard/list 非GET');
        }
    }
    ...
    public function delete() {
        log_message('info', '访问 securityguard/delete');
        ...
        if ($this->input->method(TRUE) === 'POST')
        {
            ...
            $this->form_validation->set_rules($this->msecurityguard->getPrimaryKey(), 'secuid', 'required', ['required' => '必须指定操作对象']);

            if ($this->input->is_ajax_request())
            {
                log_message('info', '访问 securityguard/delete 通过ajax');
                if ($this->form_validation->run() === TRUE)
                {
                    $id = (int) $this->input->post($this->msecurityguard->getPrimaryKey(), TRUE);
                    $result = $this->msecurityguard->delete($id);
                    if ($result)
                    {
    ...
    {
        log_message('info', '访问securityguard/detail');
        ...
        if ($this->input->method(TRUE) === 'GET')
        {
            ...
            if ($this->input->is_ajax_request())
            {
                if ($id <= 0)
                {
                    ...
                    log_message('error', '访问securityguard/detail 未提供id参数');
                }
                else
                {
                    $result = $this->msecurityguard->detail($id);
                    if ($result)
                    {
                        ...
                        if ((int) $result['photo'] > 0)
                        {
                            ...
                            if ($file)
                            {
                                $securityguardimgurl = file_web_path($file['path'], 'upload');
                            }
                            else
                            {
                                $securityguardimgurl = '';
                        ...
                        if ((int) $result['fingerprint'] > 0)
                        {
                            ...
                            if ($file)
                            {
                                $securityguardfpurl = file_web_path($file['path'], 'upload');
                            }
                            else
                            {
                                $securityguardfpurl = '';
                        ...
                        $result['securityguardimgurl'] = $securityguardimgurl;
                        $result['securityguardfpurl'] = $securityguardfpurl;
