var vm = new Vue({
    ...
    data: {
        ...
        list_url: site_url + 'securityguard/index',
        add_url: site_url + 'securityguard/add',
        edit_url: site_url + 'securityguard/edit',
        detail_url: site_url + 'securityguard/detail',
        upload_securityguardimg_url: site_url + 'upload/handle?type=securityguardimg',
        upload_securityguardfp_url: site_url + 'upload/handle?type=securityguardfp'
    },
    created: function() {
        this.ajaxDetail()
    },
    methods: {
        ...
        handleSecurityguardimgSuccess: function(res, file) {
            // 获取返回的file_id
            if (_.has(res, 'code') && 200 === res.code) {
                ...
                // 缩略图展示
                this.form.securityguardimgurl = URL.createObjectURL(file.raw);
            } else {
                this.$message({
                    type: 'error',
                    message: res.msg
        ...
        beforeSecurityguardimgUpload: function(file) {
        ...
        handleSecurityguardfpSuccess: function(res, file) {
            // 获取返回的file_id
            if (_.has(res, 'code') && 200 === res.code) {
                ...
                // 缩略图展示
                this.form.securityguardfpurl = URL.createObjectURL(file.raw);
            } else {
                this.$message({
                    type: 'error',
                    message: res.msg
        ...
        beforeSecurityguardfpUpload: function(file) {
            const isJPG = file.type === 'image/jpeg';
            const isPNG = file.type === 'image/png';
            const isLt2M = file.size / 1024 / 1024 < 2;
