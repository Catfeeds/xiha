<?php
    ...
    **/
    ...
    function parseExamRecord() {
        ...
        //验证请求参数列表
        $validate_ok = validate(array('exam_record' => 'STRING', 'user_id' => 'INT'), $request->params());
        if ( !$validate_ok['pass'] ) {
            $data = $validate_ok['data'];
            exit( json_encode($data) );
        } 
        $exam_record = $request->params('exam_record');
        ...
        try {
            ...
            $data = array('code' => '200', 'data' => $stmt);
            exit( json_encode($data) );

            //关闭数据库
            $db = null;
        } catch ( PDOException $e ) {
            setapilog('[comment_like] [:error] [client ' . $request->getIp() . '] [user_id % ' . $user_id . '] ['. $e->getLine().' ' .$e->getMessage().']');
