<?php
    ...
    **/
    ...
    function uploadExamRecord() {
        Global $app, $crypt;
        $request = $app->request();
        slimLog($app->request, $app->response, null, '');
        slimLog($app->request, $app->response, null, serialize($_FILES));
        // ready to return array $data
        $data = array();

        //验证请求方式 POST
        if ( !$request->isPost() ) {
            setapilog('[update_user_info] [:error] [client ' . $request->getIp() . '] [method % ' . $request->getMethod() . '] [106 错误的请求方式]');
        ...
        if ( $error == UPLOAD_ERR_OK ) {
            ...
            if ( $_FILES['exam_record']['type'] == 'text/plain' ) {
                ...
                setapilog('[upload_exam_record] [:error] [client ' . $request->getIp() . '] [file_type ' . $_FILES['exam_record']['type'] . ']');
                exit( json_encode($data) );
            }
        ...
        try {
            ...
            setapilog('[upload_exam_record] [:error] [client ' . $request->getIp() . '] [user_id,type % ' . $user_id . '] [1 '.$e->getMessage().']');
