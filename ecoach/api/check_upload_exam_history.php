<?php
    ...
    **/
    ...
    function checkUploadExamHistory() {
        Global $app, $crypt;
        $request = $app->request();
        ...
        //验证请求方式 POST
        if ( !$request->isPost() ) {
            setapilog('[check_upload_exam_history] [:error] [client ' . $request->getIp() . '] [method % ' . $request->getMethod() . '] [106 错误的请求方式]');
        ...
        $validate_ok = validate($ready_to_check, $request->params());
        if ( !$validate_ok['pass'] ) {
        ...
        if ( $error == UPLOAD_ERR_OK ) {
            ...
            if ( $_FILES['exam_history']['type'] == 'text/plain' ) {
                ...
                setapilog('[check_upload_exam_history] [:error] [client ' . $request->getIp() . '] [file_type ' . $_FILES['exam_history']['type'] . ']');
                exit( json_encode($data) );
            ...
            if ( !file_exists( $path ) ) {
                ...
                if ( mkdir( $path, 0777, true ) ) {
                    ...
                    setapilog('[upload_exam_record] [:error] [client ' . $request->getIp() . '] [400 创建目录失败]');
                    exit( json_encode($data) );
                }
            } else {
                //已经存在目录，移动文件到目标文件夹
            ...
            if ( $move_ok ) {
                ...
                setapilog('[upload_exam_record] [:error] [client ' . $request->getIp() . '] [400 上传保存失败]');
                exit( json_encode($data) );
            }
        }
        ...
        try {
            ...
            setapilog('[check_upload_exam_history] [:error] [client ' . $request->getIp() . '] [param ' . serialize($request->params()) . '] [' . $e->getLine() . ' '.$e->getMessage().']');
