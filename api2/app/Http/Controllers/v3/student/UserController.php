...
use App\Http\Controllers\v3\student\SmsController as Sms; // 短信
use App\Http\Controllers\v3\student\EncryptionController;
...
class UserController extends Controller {
    ...
    public function getSmsCode($user_type, $operation) {
        ...
        if (false === ($op = $this->checkOperation($operation))) {
            return ['code' => 400, 'msg' => '参数错误', 'data' => new \stdClass];
        }
        ...
        if (false === ($phone = $this->checkPhone($i_user_type, $op))) {
            return ['code' => 400, 'msg' => '手机参数错误', 'data' => new \stdClass];
        } else if (is_array($phone)) {
            return ['code' => 400, 'msg' => $phone['msg'], 'data' => new \stdClass];
        }
        ...
        try {
            ...
            if ('100' == $result['stat']) {
                ...
                return ['code' => 400, 'msg' => '发送失败,'.$result['message'], 'data' => new \stdClass];
            }
    ...
    public function register() {
        ...
        if (false === ($code = $this->checkCode($this->request, $phone, 86400))) {
            return ['code' => 400, 'msg' => '验证码失效', 'data' => new \stdClass];
        } else if (is_array($code)) {
            return ['code' => 400, 'msg' => $code['msg'], 'data' => new \stdClass];
        }
        ...
        if (! $this->request->has('pass')) {
            return ['code' => 400, 'msg' => '请填写密码', 'data' => new \stdClass];
        } else if (strlen($this->request->input('pass')) < 6) {
            return ['code' => 400, 'msg' => '密码长度不能低于6位', 'data' => new \stdClass];
        }
        ...
        if ( ! $this->request->has('identity_id')) {
            ...
            return ['code' => 400, 'msg' => '身份证格式有误', 'data' => new \stdClass];
    ...
    public function login() {
        ...
        // $pass
        if (! $this->request->has('pass')) {
            return response()->json(['code' => 400, 'msg' => '请填写密码', 'data' => new \stdClass]);
        }
        $pass = $this->request->input('pass');

        // $user_id
        if (false === ($user_id = $this->userLogin($phone, $pass, $i_user_type))) {
            return response()->json(['code' => 400, 'msg' => '密码不正确', 'data' => new \stdClass]);
        } else if (is_array($user_id)) {
            return response()->json(['code' => 400, 'msg' => $user_id['msg'], 'data' => new \stdClass]);
        }
    ...
    public function thirdLogin($user_type) {
        ...
        // $i_user_type
        if (false === ($i_user_type = $this->checkUserType($user_type))) {
            $data = ['code' => 400, 'msg' => '不允许的用户类型', 'data' => new \stdClass];
            return response()->json($data);
        }

        // $third_type
        if (false === ($third_type = $this->checkThirdType($this->request))) {
            $data = ['code' => 400, 'msg' => '参数不正确', 'data' => new \stdClass];
            return response()->json($data);
        } else if (is_array($third_type)) {
        ...
        if ($third_login) { // 已绑定
            ...
            if ( $phone_user ) { // user表中存在此用户
                ...
                if ( 0 == $i_user_type ) { // begin student
                    ...
                    $this->couponOperation('', 23, 340000, 340100, $user_info->phone, $user_info->real_name);
                // end student
                } else if ( 1 == $i_user_type ) { // begin coach
                    $phone_coach_info = $this->getCoachIdByUserId($user_id);
                    if ( !$phone_coach_info ) {
                        $phone_coach_id = DB::table('coach')
                            ->insertGetId([
    ...
    public function updateProfile() {
        ...
        $user_info = DB::table('user')
            ...
            ->first();
        $raw_data = [];
        if ($this->request->has('photo_id') && $this->request->input('photo_id') != $user_info->photo_id) {
            $raw_data['info.photo_id'] = $this->request->input('photo_id');
        }
        if ($this->request->has('user_name') && $this->request->input('user_name') != $user_info->user_name) {
            $raw_data['user.s_username'] = $this->request->input('user_name');
        }
        if ($this->request->has('real_name') && $this->request->input('real_name') != $user_info->real_name) {
            $raw_data['user.s_real_name'] = $this->request->input('real_name');
        }
        if ($this->request->has('sex') && $this->request->input('sex') != $user_info->sex) {
            $raw_data['info.sex'] = $this->request->input('sex');
        }
        if ($this->request->has('age') && $this->request->input('age') != $user_info->age) {
            $raw_data['info.age'] = $this->request->input('age');
        }
        if ($this->request->has('identity_id') && $this->request->input('identity_id') != $user_info->identity_id) {
            $raw_data['info.identity_id'] = $this->request->input('identity_id');
        }
        if ($this->request->has('address') && $this->request->input('address') != $user_info->address) {
            $raw_data['info.address'] = $this->request->input('address');
        }
        if ($this->request->has('province_id') && $this->request->input('province_id') != $user_info->province_id) {
            $raw_data['info.province_id'] = $this->request->input('province_id');
        }
        if ($this->request->has('city_id') && $this->request->input('city_id') != $user_info->city_id) {
            $raw_data['info.city_id'] = $this->request->input('city_id');
        }
        if ($this->request->has('area_id') && $this->request->input('area_id') != $user_info->area_id) {
            $raw_data['info.area_id'] = $this->request->input('area_id');
        }

        // lesson_id
        if ($this->request->has('lesson_id') && $this->request->input('lesson_id') != $user_info->lesson_id) {
            $lesson_info = DB::table('lesson_config')
        ...
        // 学车的牌照类型 license_id
        if ($this->request->has('license_id') && $this->request->input('license_id') != $user_info->license_id) {
            $license_info = DB::table('license_config')
        ...
        if ($this->request->has('exam_license_name') && $this->request->input('exam_license_name') != $user_info->exam_license_name) {
    ...
    */
    ...
    public function getMessageList() {
        ...
        $message_list = DB::table('sms_sender')
            ->select('id', 's_beizhu as beizhu', 'is_read', 'addtime', 's_content as content')
            ->where([
                ['member_id', '=', $member_id],
                ['member_type', '=', $member_type],
                ['i_yw_type', '=', $i_yw_type],
    ...
    public function exchangeCoupon() {
        ...
        // 非登陆时候
            ...
            if (false === ($_code = $this->checkCode($this->request, $phone, 86400))) {
                return ['code' => 400, 'msg' => '验证码失效', 'data' => new \stdClass];
            } else if (is_array($_code)) {
                return ['code' => 400, 'msg' => $_code['msg'], 'data' => new \stdClass];
            }
    ...
    public function bindCoach() {
        ...
        if ( ! $coach_info = $this->getCoachInfoById($coach_id) ) {
            throw new InvalidArgumentException('参数错误', 400);
        } else if ($coach_phone != $coach_info->coach_phone) {
            throw new InvalidArgumentException('参数错误', 400);
        }
    ...
    {
        $items = [];
        $items = DB::table('lesson_config')->select('lesson_id','lesson_name')->where(['is_open' => 1])->orderBy('lesson_id', 'ASC')->get();
        if(!$items) {
            $json = ['code' => 400,'msg'  => '获取失败','data' => []];
        }else {
            $json = ['code' => 200,'msg'  => '获取成功','data' => array('list'=>$items)];
        }
    ...
    **/
    ...
    public function getMyQrcode()
    {
        ...
        $data_builder = new DataBuilder();
        $qrcode_contents = $data_builder->fromTypeAndData($type, $data)->build();
        // 组合参数生成qrcode图片
        $qrcode = QrCode::format('png')
            ->errorCorrection('M') //    L:7%的字节码恢复率;   M:15%的字节码恢复率;   Q:25%的字节码恢复率;   H:30%的字节码恢复率;
            ->size(400)
            ->encoding('utf-8')
            ->margin(1)
            ->generate($qrcode_contents);
    ...
    //  获取最新版题库下载地址
    public function tikuBase($version)
    {
        $data = [];
        if(!(int)($version) > 0) {
            $data = ['code'=>400,'msg'=>'非法的版本号','data'=>new \stdClass];
        }else {
            $app_version = (int)$version;
            $last_version = (int)env('TIKU_VERSION');
            if($app_version > $last_version) {
                $data = ['code'=>400,'msg'=>'非法的版本号','data'=>new \stdClass];
            }else if($app_version == $last_version) {
                $data = ['code'=>200,'msg'=>'当前题库已经是最新版本','data'=>new \stdClass];
            }else {
                $url = env('APP_UPLOAD_PATH') . '/tiku/res.zip';
                $data = ['code'=>200,'msg'=>'已获取到最新版本下载地址','data'=>['url'=>$url,'last_version'=>(int)date('Ymd')]];
            }
        }
        return response()->json($data);
    }
