...
class ExamController extends Controller {
     ...
     * @return void
     */
    public function ranking($car_type, $course)
    {
        if(!$car_type) {
            $data = ['code' => 400,'msg'  => '车型不存在','data' => []];
        }else if(!in_array($car_type, array('car', 'truck', 'bus', 'moto', 'keyun', 'huoyun', 'weixian', 'jiaolian', 'chuzu'))){
            $data = ['code'=>400,'msg'=>'车型不正确','data'=>[]];
        }else if(!$course){
            $data = ['code'=>400,'msg'=>'科目不存在','data'=>[]];
        }else if(!in_array($course, array('kemu1', 'kemu2', 'kemu3', 'kemu4', 'zigezheng'))){
            $data = ['code'=>400,'msg'=>'科目不正确','data'=>[]];
        }else {
            ...
            $myscore = $items = $list = [];
            $time = strtotime(date('Y-m-d')) - 86400 * 110;
            DB::connection()->enableQueryLog();
            $myscore = DB::select("select `realname` as user_name, SUM(`score`) as score, SUM(`exam_total_time`) as time, `cs_users_info`.`user_photo` from `cs_user_exam_records` LEFT JOIN `cs_users_info` ON `cs_user_exam_records`.`user_id`=`cs_users_info`.`user_id` WHERE `cs_user_exam_records`.`addtime`>".$time." AND `cs_user_exam_records`.`user_id`='{$user_id}' AND `car_type`='{$car_type}' AND `course`='{$course}'");
            $queries = DB::getQueryLog();

            $items = DB::select("select `realname` as user_name, SUM(`score`) as score, SUM(`exam_total_time`) as time, `cs_users_info`.`user_photo` from `cs_user_exam_records` LEFT JOIN `cs_users_info` ON `cs_user_exam_records`.`user_id`=`cs_users_info`.`user_id` WHERE `cs_user_exam_records`.`addtime`>".$time."  AND `car_type`='{$car_type}' AND `course`='{$course}' GROUP BY `user_name` ORDER BY `score` DESC LIMIT 100");

            if($items) {
                foreach($items as $k=>$v) {
                    $list[$k]['No.'] = $k + 1;
                    $list[$k]['user_photo'] = $v->user_photo;
                    $list[$k]['user_name'] = $v->user_name;
                    $list[$k]['score'] = $v->score . '分';
                    $list[$k]['time'] = $this->dataformat($v->time);
                }
            }

            if(!$myscore[0]->user_name && !$myscore[0]->user_photo && !$myscore[0]->score && !$myscore[0]->time) {
                $myscore = [];
            }

            $data = ['code' => 200,'msg'  => '成功','data' => ['myscore'=>$myscore,'list' => $list]];
        }
