...
class SignupController extends Controller {
    ...
    public function getIndexRecommendSchoolList () {
        ...
        if ($school_list) {
            foreach ($school_list as $key => $value) {
                ...
                // 是否热门  是否是热门驾校（1：是 | 2：否）
                if(2 == $value->is_hot) {
                    $school_list[$key]->is_hot = 0;
                }
            }
        }
    ...
    //  获取驾校所有班制信息
    public function getSchoolShiftsById ($school_id)
    {
        if(!$school_id) {
            $data = ['code'  => 400,'msg'   => '参数错误','data'  => new \stdClass];
        }else if(!$detail = DB::table('school')->select()->where(['l_school_id'=>$school_id])->get()) {
            $data = ['code'  => 400,'msg'   => '驾校不存在','data'  => new \stdClass];
        }else {
            $where = function ($query) {$query->whereNull('coach_id')->orWhere('coach_id', '=', 0)->orWhere('coach_id', '=', '');};
            ...
            if ($shifts_info) {
                foreach ($shifts_info as $key => $value) {
                    ...
                    // }
                    // 已报名人数
                    $shifts_info[$key]->student_count = 0;
                    ...
                    $shifts_info[$key]->student_count = $student_count_rlt[0]->cnt;
                    unset($shifts_info[$key]->is_package);
                    unset($shifts_info[$key]->sh_type);
                    unset($shifts_info[$key]->sh_license_id);
                }
                $data = ['code'  => 200,'msg'   => '获取成功','data'  => ['list'=>$shifts_info]];
            }else {
                $data = ['code'  => 200,'msg'   => '该驾校暂无班制信息','data'  => new \stdClass];
            }
        }
        return response()->json($data);
    }
    ...
    private function getSchoolShiftsByCount ($school_id, $count = 0) {
        ...
        if ($shifts_info) {
            foreach ($shifts_info as $key => $value) {
                ...
                $student_count_rlt = DB::select($student_count_sql);
                $shifts_info[$key]->student_count = $student_count_rlt[0]->cnt;
                unset($shifts_info[$key]->is_promote);
                unset($shifts_info[$key]->sh_school_id);
                unset($shifts_info[$key]->sh_license_id);
                unset($shifts_info[$key]->sh_type);
            }
    ...
    public function getIndexRecommendCoachList() {
        ...
        $whereCondition = [
            ...
            ['coach.certification_status', [1, 3]]
        ];
        ...
        foreach ($coach_list->toArray() as $key => $value) {
            ...
            $coach_list[$key]->student_count = 0;
            $student_count_sql = "select count(1) as cnt from `cs_study_orders` where `cs_study_orders`.`l_coach_id`= " . $value->coach_id . " and `cs_study_orders`.`dt_zhifu_time`!=''";
            $student_count_rlt = DB::select($student_count_sql);
            $coach_list[$key]->student_count = $student_count_rlt[0]->cnt;
            if(2 == $value->is_hot) {
                $coach_list[$key]->is_hot = 1;
            }else {
                $coach_list[$key]->is_hot = 0;
            }
            unset($coach_list[$key]->timetraining_supported);
        }
        
        $data = ['code'=>200, 'msg'=>'获取推荐教练成功', 'data'=>['list'=>$coach_list]];
        return response()->json($data);
    ...
    public function getSchoolMessage () {
        ...
        $lng = $this->request->input('lng'); // 经度
        $lat = $this->request->input('lat'); // 纬度
        $detail = $this->getSchoolDetail($school_id);
        if(!$detail->school_name) {
            return response()->json([
                'code' => 400,
                'msg'  => '驾校不存在',
                'data' => ''
            ...
            // 获取驾校班制
            $shifts_list = $this->getSchoolShiftsByCount($school_id, 0);
            $_data['shifts_info'] = $shifts_list;
            ...
            // 驾校星级
            if ($comment_list && $comment_list->average_star) {
                $_data['school_info']->school_star = $comment_list->average_star;
            } else {
                $_data['school_info']->school_star = 3;
            }
            unset($_data['school_info']->tl_location_x);
            unset($_data['school_info']->tl_location_y);
            unset($_data['school_info']->is_show);
            unset($_data['school_info']->brand);
            unset($_data['school_info']->school_imgurl);
            unset($_data['school_info']->tl_train_address);
            unset($_data['school_info']->tl_phone);
            unset($_data['school_info']->tl_imgurl);
            unset($_data['school_info']->min_distance);
            unset($_data['school_info']->distance_unit);
            $data = [
                'code'  =>200,
                'msg'   =>'获取驾校详情成功',
                'data'  =>$_data
    ...
    public function getSchoolDetail($school_id) {
        ...
        if($school_detail) {
            ...
            $school_detail->school_id = '';
                $school_detail->sschool_name ='';
                $school_detail->sschool_imgurl ='';
                $school_detail->sis_show ='';
                $school_detail->slocation_x ='';
                $school_detail->slocation_y ='';
                $school_detail->sschool_phone ='';
                $school_detail->sschool_desc ='';
                $school_detail->sschool_address ='';
                $school_detail->sbrand ='';
                $school_detail->sthumb_imgurl ='';;
            return $school_detail;
        }
    }
    ...
    public function getSchoolCommentList($school_id, $type=1) {
        ...
        if($_data->average_star > 5) {$_data->average_star = 5;}
        return $_data;
    }
