...
use App\Http\Controllers\v1\CoachController;
use App\Http\Controllers\v1\UserController;
use App\Http\Controllers\v1\WechatpayController;
...
class OrderController extends Controller {
    ...
    public function getMySignupOrdersList() {
        ...
        case 3: // 已完成
            $whereCondition = function($query) {
                $query->where('school_orders.so_order_status', '=', '5')
                    ->whereIn('school_orders.so_pay_type', [1, 3, 4])
                    ->orWhere( function($_query) {
        ...
        $data = ['code'=>200, 'msg'=>'获取订单成功', 'data'=>$order_list];
    ...
    public function submitOrder($order_type) {
        ...
        if('signup' == $order_type) {
            if(!$this->request->has('real_name')
                ...
                || !$this->request->has('shifts_id')
                || !$this->request->has('school_id')
                || !$this->request->has('coach_id')
                // || !$this->request->has('licence')
                || !$this->request->has('sh_category')
            ) {
            ...
            $sh_category    = $this->request->input('sh_category');
    ...
    private function signupOrderSubmitOperate($params) {
        ...
        $coupon_id      = $params['coupon_id'] ? $params['coupon_id'] : 0;
        ...
        $sh_category    = $params['sh_category'] ? $params['sh_category'] : 1;  // 班制分类 1:驾校设置的 2:教练设置的
        $pay_type       = 1;  // 支付宝
        ...
        if($coupon_id) {
            // 通过优惠券ID获取优惠券信息
            // 获取教练设置的券
            $coupon_info = DB::table('user_coupon')
                ->select('coupon_name', 'coupon_value', 'coupon_code')
                ->where([
                    'user_phone' => $phone,
                    'coupon_sender_owner_id' => $coach_id,
                    'coupon_sender_owner_type' => 1,
                    'id' => $coupon_id
                ])
                ->first();
            if(empty($coupon_info)) {
                throw new InvalidArgumentException('你选择的优惠券不是当前教练所设置的，不能使用', 400);
            }
            $coupon_name = isset($coupon_info->coupon_name) ? $coupon_info->coupon_name : '';
            $coupon_value = isset($coupon_info->coupon_value) ? $coupon_info->coupon_value : 0;
            $coupon_code = isset($coupon_info->coupon_code) ? $coupon_info->coupon_code : '';
        }
    ...
    public function payOrder() {
        ...
        switch ($pay_type) {
        case 1:
            $this->pay = new PayController(1);
            $_pay = $this->pay->purchase($pay_param);
            $_err = ( ! is_array($_pay)) ? TRUE : FALSE;
            $_err_msg = (is_string($_pay)) ? $_pay : '';
            $pay_info['alipay'] = $_pay;
            break;
        case 3:
            $this->pay = new PayController(3);
            $_pay = $this->pay->purchase($pay_param);
            $_err = ( ! is_array($_pay)) ? TRUE : FALSE;
            $_err_msg = (is_string($_pay)) ? $_pay : '';
            $pay_info['wxpay'] = $_pay;
            break;
        case 4:
            $this->pay = new PayController(4);
            $_pay = $this->pay->purchase($pay_param);
            $_err = ( ! is_array($_pay)) ? TRUE : FALSE;
            $_err_msg = (is_string($_pay)) ? $_pay : '';
            $pay_info['unionpay'] = $_pay;
            break;
        case 2:
        default :
    ...
    public function getCancelReason($type) {
        ...
        $data = ['code' => 200, 'msg' => '成功', 'data' => ['reason_list' => $reason_list], ];
        return response()->json($data);;
    }
