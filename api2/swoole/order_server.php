...
$server = new swoole_server("0.0.0.0", 5511, SWOOLE_BASE, SWOOLE_SOCK_TCP);
    ...
    if ( $worker_id >= $server->setting['worker_num'] ) {
        ...
        if ( $worker_id == 0 ) {
            ...
            $server->tick( 1000, function() use ( $server, $worker_id ) {
                ...
                $redis->auth('dalinux');
                $queue_length = $redis->get('appoint_order_all_queue_length');
                $list_length = $redis->llen('appoint_order_all');
                if ( $list_length != $queue_length ) {
                    if ( $list_length <= 0 ) {
                        $list_length = 0;
                    }
                    echo date('Y-m-d H:i:s').' 当前排队长度：'.$list_length."\n";
                    $redis->set('appoint_order_all_queue_length', $list_length);
                }
            });
        } else {
            ...
            $server->tick( 200, function() use ( $server, $worker_id ) {
                ...
                $list_length = $redis->llen('appoint_order_all');
                while ( $list_length > 0 ) {
                    $pdata = $redis->lpop('appoint_order_all');
                    if ( $pdata ) {
                        $order_info = json_decode( $pdata );
                        if ( ($order_info->addtime + 180) <= time() ) {
                            // 订单超时
                        ...
                        // 还没有超时，插入到队列前头
                        $redis->lpush('appoint_order_all', $pdata);
                        // 跳出循环 while
                        break;
                    }
                }
