		...
		if ($_SERVER['REQUEST_METHOD']=="POST") {
            if (!empty(I('post.captcha'))) {
                ...
                if(!$vry->check(I('post.captcha'))){    //如果验证码错误
                    ...
                    //提交登录数据
            		$data['user_account']=I('post.username');             //将用户名存入数组
            		$data['user_password']=md5(I('post.password'));    //将密码存入数组
            		$user = M('user')->where($data)->find();       //将用户名和密码组成的数组作为条件在数据库中查找
            		if ($user==null) {                            //如果$user为空
                        ...
                        //将用户登陆信息赋给变量
                        $session = $user['user_account'].'\t'.$user['id'].'\t'.$user['user_password'].'\t'.$user['group_id'].'\t'.$user['user_name'].'\t'.$user['user_phone'].'\t'.$user['first_login'].'\t'.$user['last_login'];
                        $loginauth = $crypt->encrypt($session, CRYPT_KEY);//用Crypt加密类加密方法加密
                        //将加密后的用户登陆信息保存在session中
                        session('loginauth', $loginauth);
                        //记录行为
                        // action_log('login', 'user', $user['id'], $user['id']);
                        $this->redirect('Index/index'); //登陆后的首页    
            		}
                }
            }
        }else{
