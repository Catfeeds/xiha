...
class AdsLevelModel extends BaseModel{
    ...
    */
    public function getAllRecords() {
        ...
        $ads_level = $this->limit($Page->firstRow . ',' . $Page->listRows)->order('level_id DESC')->select();
        $_data = array();
    ...
    public function updateOneRecordPartial( $p = array() ) {
        ...
        if ( isset($p['level_id']) ) {
            ...
            if ( !is_numeric($p['level_id']) ) {
                ...
                $old_level_id = $this->where(array('id' => $p['id']))
                                    ->getField('level_id');
        ...
        $ads_level = D('ads_level');
        if ($res = $ads_level->create($ads_level)) {
            $result = $ads_level->where(array('id' => $p['id']))
                                ->save();
            if ($result) {
                return 200;
            } else {
    ...
    **/
    public function addOneRecord($p) {
        $ads_level = D('ads_level');
        if ($res = $ads_level->create($p)) {
            $result = $ads_level->fetchSql(false)->add($res);
