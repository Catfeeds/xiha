	...
	<div class="iframe-content">
		...
		<div class="gx-iframe-content">
			<div class="gx-iframe-operation" style="margin-bottom: 20px; border-radius: 4px;">
				<el-button type="success" style="margin-left:10px;" @click.active.prevent="handleAdd($event)" id="add" data-title="添加管理员"><i class="el-icon-plus"></i> 添加管理员</el-button>
			...
			<el-table :data="list" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" >
				...
				<el-table-column prop="content" label="用户名称" show-overflow-tooltip></el-table-column> 
				...
				<el-table-column prop="s_rolename" label="角色名称" show-overflow-tooltip>
					<template scope="scope">
						<div v-if="scope.row.s_rolename">
							<el-tag type="danger" v-if="scope.row.role_id == '1'">{{ scope.row.s_rolename }}</el-tag>
							<el-tag type="primary" v-else>{{ scope.row.s_rolename }}</el-tag>
						</div>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" fixed="right">
					<template scope="scope">
						<a title="删除" style="margin-left:8px;" v-if="scope.row.id != '1'" @click="handleDel(scope.row.id, scope.$index, list)"><i class="el-icon-delete"></i></a>
...
<script>
	...
	var vm = new Vue({
		...
		data: {
			...
			show_url: "<?php echo base_url('admin/handleShow'); ?>",
			currentPage: 1,
