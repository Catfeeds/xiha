	...
	<div class="iframe-content">
		...
		<div class="gx-iframe-content">
			...
			<el-table :data="list" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" >
				...
				<el-table-column prop="i_order" label="排序" width="100" ></el-table-column> 
                ...
                <el-table-column prop="is_top" label="顶部展示" min-width="130" show-overflow-tooltip>
					<template scope="scope">
						<el-button size="small" @click="showTopStatus(scope.row.moduleid, 2)" type="success" v-if="parseInt(scope.row.is_top) == 1">是</el-button>
						<el-button size="small" @click="showTopStatus(scope.row.moduleid, 1)" type="warning" v-if="parseInt(scope.row.is_top) == 2">否</el-button>
					</template>
                </el-table-column>
				...
				<el-table-column prop="m_directory" label="说明" min-width="180" ></el-table-column>
				<el-table-column prop="m_pagecode" label="唯一码" min-width="140" ></el-table-column>
				<el-table-column label="操作" fixed="right" min-width="150">
					<template scope="scope">
						...
						<el-button size="small" style="color: #333" type="text" data-title="编辑菜单" @click.native.prevent="handleEdit($event, scope.row.moduleid, scope.$index, scope.row)"><i class="el-icon-edit"></i></el-button>
					</template>
				</el-table-column>
			</el-table>
...
<script>
	...
	var vm = new Vue({
		...
		data: {
			...
			top_url: "<?php echo base_url('admin/showTopStatus'); ?>",
			currentPage: 1,
		...
		methods: {
            ...
            showTopStatus: function(id, status) {
				$.ajax ({
                    type: 'post',
                    url: this.top_url,
                    data: {id: id, status: status},
