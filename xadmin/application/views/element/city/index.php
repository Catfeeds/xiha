<div id="app" v-cloak v-loading.fullscreen.lock="fullscreenLoading" element-loading-text="拼命加载中">
	<div class="iframe-content">
        ...
        <!--search-->
        <div class="gx-search">
            <el-form :inline="true" v-model="search" :rules="rules">
                <el-form-item label="热门状态" required>
                ...
                <el-form-item label="城市信息" required>
        ...
        <div class="gx-iframe-content">
            ...
            <el-table :data="list" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
				...
				<el-table-column prop="area_id" label="ID" sortable width="80"  ></el-table-column> 
                ...
                <el-table-column prop="area" label="区名称 | 区号" min-width="200" show-overflow-tooltip>
                    <template scope="scope">
                        <span>{{ scope.row.area }}</span> | 
                        <span style="color: red">{{ scope.row.areaid }}</span>
                    </template>
                </el-table-column> 
				<el-table-column prop="acronym" label="市缩写" width="130"></el-table-column> 
				<el-table-column prop="spelling" label="市全拼" width="150"></el-table-column> 
                <el-table-column prop="is_hot" label="是否热门" width="130" :filters="hotfilters" :filter-method="filterTag">
					<template scope="scope">
				        <el-button type="info" @click="handleHotStatus(scope.row.city_id, 2)" v-if="parseInt(scope.row.is_hot) == 1" size="small">是</el-button>
				        <el-button type="warning" @click="handleHotStatus(scope.row.city_id, 1)" v-if="parseInt(scope.row.is_hot) == 2" size="small">否</el-button>
			      	</template>
				</el-table-column>
				<el-table-column label="操作" fixed="right" width="140">
					<template scope="scope">
						<a title="删除" style="margin-left:8px;" @click="handleDel(scope.row.area_id, scope.$index, list)"><i class="el-icon-delete"></i></a>
						<a title="编辑" data-title="更新城市" style="margin-left:8px;" @click="handleEdit($event, scope.row.area_id, scope.$index, scope.row)"><i class="el-icon-edit"></i></a>
					</template>
				</el-table-column>
			</el-table>
            </el-table>
...
<script>
	...
	var vm = new Vue({
		...
		data: {
            ...
            rules: {
                is_hot: [
                    { required: true, message: '请选择热门状态', trigger: 'blur' }
                ],
                kewords: [
                    { required: true, message: '请输入城市信息', trigger: 'blur' }
                ],
            },
            list_url: "<?php echo base_url('city/listAjax')?>",
            hot_url: "<?php echo base_url('city/handleHotStatus')?>",
            add_url: "<?php echo base_url('city/add')?>",
            edit_url: "<?php echo base_url('city/edit')?>",
            del_url: "<?php echo base_url('city/delAjax')?>?name=area",
            currentPage: 1,
            ...
            delAjax: function(id) {
                $.ajax({
                    ...
                    success: function(data) {
                        is_ok = _.isObject(data) && _.has(data, 'code') && _.get(data, 'code') == 200
                        if (is_ok) {
                            var filters = {"p": vm.currentPage, "hot": vm.search.is_hot, "keywords": vm.search.keywords, "s": vm.page_size};
                            vm.listAjax(filters);
                        }
                    },
                    error: function(e) {
