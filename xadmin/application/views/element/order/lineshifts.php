					...
					<el-tab-pane label="全部" name="zero">
							<el-table :data="all_list" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" >
								...
								<el-table-column prop="so_order_status" label="订单状态" min-width="100" show-overflow-tooltip>
									<template scope="scope">
										<el-tag type="primary" v-if="scope.row.so_order_status == '3'">已付款</el-tag>
										<el-tag type="warning" v-else-if="scope.row.so_order_status == '4'">退款中</el-tag>
										<el-tag type="danger" v-else-if="scope.row.so_order_status == '2'">已取消</el-tag>
										<el-tag type="warning" v-else-if="scope.row.so_order_status == '1'">未付款</el-tag>
...
<script>
	...
	var vm = new Vue({
		...
		data: {
			...
			set_url: "<?php echo base_url('order/setOrderAjax'); ?>?ot=line",
			currentPage: 1,
		...
		created: function() {
			var filters = {'p': this.currentPage, 'keywords':this.search.keywords, 'os': this.order_status, 'ot': this.online_type, 's': this.page_size};
		...
		methods: {
			...
			handleCurrentChange: function (val) {
				...
				var filters = {'p': this.currentPage, 'keywords':vm.search.keywords, 'os': vm.order_status, 'ot': vm.online_type, 's': this.page_size};
                vm.listAjax(filters);
            },
