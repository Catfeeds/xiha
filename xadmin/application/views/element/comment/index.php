		...
		<div class="gx-iframe-content">
			...
			<el-table :data="list" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" >
				...
				<el-table-column prop="" label="评价入口" width="160">
					<template scope="scope">
						<el-tag size="small" type="success" >预约学车</el-tag>
					</template>
				</el-table-column>

				<el-table-column prop="coach_star" label="驾校星级" width="145" show-overflow-tooltip>
					<template scope="scope" >
						<el-div style="color:red;" type="danger" v-if="parseInt(scope.row.coach_star) == 1" close-transition>★</el-div>
						<el-div style="color:red;" type="danger" v-else-if="parseInt(scope.row.coach_star) == 2" close-transition>★★</el-div>
						<el-div style="color:red;" type="danger" v-else-if="parseInt(scope.row.coach_star) == 3" close-transition>★★★</el-div>
						<el-div style="color:red;" type="danger" v-else-if="parseInt(scope.row.coach_star) == 4" close-transition>★★★★</el-div>
						<el-div style="color:red;" type="danger" v-else close-transition>★★★★★</el-div>
					</template>
				</el-table-column>
				<el-table-column prop="coach_content" label="评价内容" width="150" show-overflow-tooltip></el-table-column>
...
<script>
	...
	var vm = new Vue({
		...
		data: {
			...
			list: [],
			multipleSelection: [],
			list_url: "<?php echo base_url('comment/listajaxStuCommentCoach'); ?>",
			del_url: "<?php echo base_url('comment/delStuCommentCoach'); ?>",
			edit_url: "<?php echo base_url('comment/editStuCommentCoach'); ?>",
			preview_url: "<?php echo base_url('comment/preStuCommentCoach'); ?>",
			show_url: "<?php echo base_url('comment/showStuCommentCoach'); ?>",
			search_url: "<?php echo base_url('comment/searchStuCommentCoach'); ?>",
			currentPage: 1,
