...
//电商ID
defined('EBusinessID') or define('EBusinessID', '1298955');
//电商加密私钥，快递鸟提供，注意保管，不要泄漏
defined('AppKey') or define('AppKey', '751893e0-eff3-433e-abd1-2e15fe67488b');
//请求url
...
class OrderController extends Controller
{
	...
	public function init()
	{
        ...
        $this->assign('mainMenu', $mymenu_items);
		$menu_index = $menu->current_menu();
		$this->assign('menu_index',$menu_index);
		$subMenu_items = $menu->getSubMenu($menu_index['menu']);
		foreach($subMenu_items as $k=>$v) {
			if($v['name'] == '单据管理'){
				unset($subMenu_items[$k]);
			}
		}
		$this->assign('subMenu',$subMenu_items);
	...
	public function doc_invoice_save(){
		...
		$express_code = Filter::str(Req::args("express_code"));
		$express_company_id = Filter::str(Req::args('express_company_id'));
		...
		if($order_info){
			...
			$payment_id = $order_info['payment'];
				...
				$express_company = $model->table('express_company')->where('code='."'" . $express_company_id . "'")->find();
		...
		$this->model->table('order_log')->data($log)->insert();
		echo "<script>parent.send_dialog_close();</script>";
	}

	//	查询订单物流轨迹
	public function order_traces()
	{
		$id = Req::args("id");
		$express = $this->model->table('doc_invoice')->where('id='.$id)->find();
		$no = $express['express_code'];
		$code = $express['express_no'];
		$requestData= "{'OrderCode':'','ShipperCode':'{$no}','LogisticCode':'{$code}'}";
		$datas = array(
	    ...
	    fclose($fd);
	    $rlt = json_decode($gets, true);
	    if($rlt['Success']) {
	    	$this->layout = "blank";
	    	$this->assign("items",$rlt['Traces']);
			$this->redirect();
	    }
	}
	...
	//	订单支付日志统计
	public function payment_log_tongji()
	{
	...
	//	订单支付日志列表
	public function payment_log_list()
	{
		$condition = Req::args("condition");
		$this->assign("condition",$condition);
		$condition_str = Common::str2where($condition);
	...
	//	删除订单支付日志
	public function payment_log_del(){
		$id =  Req::args('log_id');
		//删除
		if(is_array($id)){
			$ids = implode(",", $id);
		...
		// 	if ($ref_route!=''){
		// 		$this->redirect($ref_route,true,array('msg'=>$msg));
		// 	}else{
	...
	public function order_list(){
		...
		$this->assign("pay_status",array('0'=>'<span class="red">未付款</span>','1'=>'<b style="color:green;">已付款</b>','2'=>'申请退款','3'=>'已退款'));
		$this->assign("delivery_status",array('0'=>'<span class="red">未发货</span>','1'=>'<b style="color:blue;">已发货</b>','2'=>'<b style="color:green;">已签收</b>','3'=>'申请换货','4'=>'已换货'));
