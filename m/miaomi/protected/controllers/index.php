...
class IndexController extends Controller{
    ...
    public function sendsms()
    {
        $mobile = $_POST['mobile'];
        if(!$mobile) {
           echo JSON::encode(array('status'=>0,'msg'=>'请输入手机号'));
        }else if(!$this->is_mobile($mobile)) {
           echo JSON::encode(array('status'=>0,'msg'=>'手机号码格式错误'));
        }else {
            $code = rand(100000,999999);
            $template = 405968;
            Session::set('code_'.$mobile,$code,'300');
            require_once(TINY_ROOT . "/extend/Sms/sms.php");
            $sms_obj = new SmsApi();
            $rlt = $sms_obj->send($mobile, array('code'=>$code),$template);
            if($rlt) {
    ...
    //  商品详情页(即购买页面) 2017-07-03 18:14:14 by xiayufeng
    public function products()
    {
        $this->assign("_UnitPrice", 199);
        $this->assign("_UnitPrice2", 199);
        $this->assign("_Freight", 10);
        $this->redirect();
    }

    //  软件下载 2017-07-04 17:05:54 by xiayufeng
    ...
    public function order() {
        $data = $_POST;
        if(!$data['accept_name']) {
            echo JSON::encode(array('status'=>0,'msg'=>'请输入姓名'));
        }else if(!$data['mobile']){
            echo JSON::encode(array('status'=>0,'msg'=>'请输入手机号'));
        }else if(!$this->is_mobile($data['mobile'])) {
            echo JSON::encode(array('status'=>0,'msg'=>'手机号码格式不正确'));
        }else if(!$data['smscode']) {
            echo JSON::encode(array('status'=>0,'msg'=>'请输入验证码'));
        }else if($data['smscode'] != Session::get('code_'.$data['mobile'])) {
            echo JSON::encode(array('status'=>0,'msg'=>'验证码不正确'));
        }else if(!$data['province']) {
            echo JSON::encode(array('status'=>0,'msg'=>'请选择省份'));
        }else if(!$data['city']) {
            echo JSON::encode(array('status'=>0,'msg'=>'请选择城市'));
        }else if(!$data['county']) {
            echo JSON::encode(array('status'=>0,'msg'=>'请选择区县'));
        }else if(!$data['addr']) {
            echo JSON::encode(array('status'=>0,'msg'=>'请输入详细地址'));
        }else if(!$data['pay_code']) {
            echo JSON::encode(array('status'=>0,'msg'=>'请选择支付方式'));
        }else if((float)$data['price'] < 199) {
            echo JSON::encode(array('status'=>0,'msg'=>'订单金额不正确'));
        }else {
            $order = array(
            ...
            if($order_id = $this->model->table("order")->data($order)->insert()) {
                $order_goods = array(
                    'order_id'=>$order_id,
                    'goods_id'=>0,
                ...
                if($id = $this->model->table("order_goods")->data($order_goods)->insert()) {
                    echo JSON::encode(array('status'=>1,'msg'=>'下单成功','data'=>array('order_id'=>$id,'pay_code'=>$data['pay_code'])));
                }else {
                    echo JSON::encode(array('status'=>0,'msg'=>'下单失败111'));
                }
            }else {
                echo JSON::encode(array('status'=>0,'msg'=>'下单失败222'));
            }
        }
    }
    ...
    function index(){
        $this->assign("_Freight", 15);
        $this->assign("_UnitPrice", 200);
        $this->redirect();
    }
