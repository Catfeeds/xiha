...
class AdminController extends Controller
{
    ...
    public function class_config_save()
    {
        $this->checkFormToken();
            ...
            if ($dh = opendir($theme_path)) {
                while (($file = readdir($dh)) !== false) {
                    if ($file != '.' && $file != '..' && is_dir($theme_path . $file)) {
                        ...
                        $theme_config_file = $theme_path . $file . "/config.php";
                        if (is_file($theme_path_file)) {
                            ...
                            if (isset($info['type']) && $info['type'] == 'mobile') {
                                $themes['mobile'][] = array('file' => $file, 'info' => $info, 'config' => $config);
                            } else {
                                $themes['pc'][] = array('file' => $file, 'info' => $info, 'config' => $config);
                            }
    ...
    public function theme_config()
    {
        ...
        $theme_path        = APP_ROOT . '/themes/';
        $theme_config_file = $theme_path . $theme_name . "/config.php";
        $config            = null;
        if (is_file($theme_config_file)) {
            $config = include $theme_config_file;
        }
        $model         = new Model('theme_config');
        $obj           = $model->where("name='$theme_name'")->find();
        $config_values = array();
        if ($obj != null) {
            $config_values = unserialize($obj['config']);
        }
        $this->assign('config_values', $config_values);
        $this->assign('config', $config);
        $this->assign('theme', $theme_name);
        $this->redirect();
    }

    public function theme_config_save()
    {
        $theme_name        = Req::args('theme');
        $theme_path        = APP_ROOT . '/themes/';
        $theme_config_file = $theme_path . $theme_name . "/config.php";
        ...
        if (is_file($theme_config_file)) {
            ...
            foreach ($config as $value) {
                ...
                if ($value['type']=='checkbox') {
                    $config_values['theme_' . $value['field']] = implode(',',Req::args('theme_' . $value['field']));
                } else {
                	if (method_exists("Filter", $filter)){
                		$config_values['theme_' . $value['field']] = Filter::$filter(Req::args('theme_' . $value['field']));
                	}else{
                		$config_values['theme_' . $value['field']] = Req::args('theme_' . $value['field']);
                	}
                }
            ...
            $obj           = $model->where("name='$theme_name'")->find();
            if ($obj) {
                $model->data(array('config' => $config_values))->where("name='$theme_name'")->update();
            } else {
                $model->data(array('name' => $theme_name, 'config' => $config_values))->add();
            }
        }
        $this->redirect("/admin/theme_config/theme/$theme_name/opstatus/success");
    ...
    //权限资源保存
    public function resources_save()
    {
        $this->checkFormToken();
        $id    = Req::args("id");
        $right = Req::args("right");
    ...
    public function optimize_table(){
        ...
        if($tables){
            foreach ($tables as $table) {
                $table = $table;
                $model->query('optimize table '.$table.';');
            }
            $this->msg = array("success", '整理优化完成');

        }else{
            $this->msg = array("warning", '您没有选择需要优化表');
        }
        $this->redirect("tables_list", false);
    }
    //备份下载操作
    public function down()
    {
        ...
        $flag = (bool)preg_match('/^[a-zA-Z0-9_]+\.sql$/i',$backs);
        if($flag){
    ...
    public function zip()
    {
        ...
        if (!empty($backs)) {
            if (is_array($backs)) {
                $len = count($backs);
                for ($i = 0; $i < $len; $i++) {
                    $flag = (bool)preg_match('/^[a-zA-Z0-9_]+\.sql$/i',$backs[$i]);
                    if($flag){
    ...
    //上传SQL文件恢复
    public function upload_recover()
    {
        $upfile_path = Tiny::getPath("uploads");
