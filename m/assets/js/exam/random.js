(function($,doc) {
    ...
    var limit = 1;
    var qid = 1;
//  var page = localStorage.getItem('temporary_page') ? localStorage.getItem('temporary_page') : 1;
    var page = 1;
    page = page == 0 ? 1 : page;
    ...
    var localstorage_name = 'random_id';
	...
	questions_no_list.addEventListener('tap', function() {
    	...
    	var localstorage_name = 'random_id';
    	var question_list = localStorage.getItem('question_list') ? JSON.parse(localStorage.getItem('question_list')) : [];
		var random_id_list = localStorage.getItem('random_id_'+ctype+'_'+stype) ? JSON.parse(localStorage.getItem('random_id_'+ctype+'_'+stype)) : [];
...
//		var curr_qid = localStorage.getItem('curr_qid_'+ctype+'_'+stype);
		...
		$.each(random_id_list, function(k, v) {
			if(curr_qid == v.id) {
				random_id_list[k]['status'] = '2';
    		} else {
				random_id_list[k]['status'] = '1';
		    }
			if(-1 != errq_list.indexOf(parseInt(v.id))) {
				random_id_list[k]['status'] = '4';
    		}
			if(-1 != trueq_list.indexOf(parseInt(v.id))) {
				random_id_list[k]['status'] = '3';
    		}
		});
		$.each(random_id_list, function(k, v) {
			switch(v.status) {
