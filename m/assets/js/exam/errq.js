...
//	var errq = localStorage.getItem('errq_'+ctype+'_'+stype) ? JSON.parse(localStorage.getItem('errq_'+ctype+'_'+stype)) : [];
	if(errq.length == 0) {
		doc.getElementById('question').innerHTML = '<div class="" style="background: #f5f5f5; text-align:center; padding-top:70px; margin:0px auto;"><span class="iconfont" style="color:#E4E4E4; font-size:5rem;">&#xe63b;</span><br/><p style="text-align: center; margin-top:25px;">你真棒，还没有错题哦！</p></div>';
		return false;
	}
    ...
    // 获取题号
  	localStorage.setItem('qtotal', errq.length);
  	
    // 获取分页的题目号
    var _question_list = getQuestionIdsByErrQ(page, limit, errq);
    ...
    question_list.qtotal = errq.length;
    doc.getElementById('qtotal').innerHTML = errq.length;
	...
	questions_no_list.addEventListener('tap', function() {
		...
		var trueq_id_list = localStorage.getItem('temporary_trueq') ? JSON.parse(localStorage.getItem('temporary_trueq')) : [];
    	...
    	var list = [];
    	$.each(errq_id_list, function(k, v) {
    		list[k] = {
    			'id':v
    		};
    	});
		...
		var title = '<span class="dacuo">错误：<b id="dacuo_num" style="color:#f25e5e;">'+errq_id_list.length+'</b></span>';
    	html += '</div>';
	 	layer.open({
            type: 1,
            content: html,
	...
	// 从错题中获取题目ID
	function getQuestionIdsByErrQ(page, limit, errq) {
	    if(typeof errq != 'object' || errq.length == 0) {
	        return false;
	    }
	   	var p = page <= 0 ? 1 : page;
	    var start = (p - 1) * limit;
