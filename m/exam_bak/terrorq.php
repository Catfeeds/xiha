...
<html>
	...
	<body style="background: #f5f5f5;">
		<header class="mui-bar mui-bar-nav">
			<a href="javascript:history.go(-2)" class="mui-icon mui-icon-left iconfont" style="color: #00BD9C; font-size:1.3rem;">&#xe608;</a>
			<h1 class="mui-title" id="">模拟错题</h1>
		</header>
...
//		        console.log(qid);
				...
				next.addEventListener('tap', function(e) {
					...
					var cids = !localStorage.getItem('terrq') ? '' : localStorage.getItem('terrq');
				...
				$('#question').on('tap', '.mui-ul-click .mui-single', function() {
					...
					if(title == _question.answertrue) {
						...
						localStorage.setItem('terrq', terrq.replaceAll(_question.id + ",", ""));
						...
						//记录错题
//						var terrq = !localStorage.getItem('terrq') ? '' : localStorage.getItem('terrq');
//						localStorage.setItem('terrq', terrq + _question.id + ',');
					}
					l
					c++;
				});
